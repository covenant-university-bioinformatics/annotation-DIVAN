...
import {InputType} from "../models/divan.model";
...
export const validateInputs = async (
  ...
  if(createJobDto.input_type === InputType.VARIANTBASED){
    const numberColumns = [
      'marker_name',
    ];
    ...
    try {
      ...
      columns = numberColumns.map((column) => parseInt(createJobDto[column], 10));
    } catch (e) {
      console.log(e);
  ...
  if(createJobDto.input_type === InputType.REGIONBASED){
    const numberColumns = [
      'chr',
      'start_position',
      'end_position',
